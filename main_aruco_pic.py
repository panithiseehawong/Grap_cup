...
def process_snapshot_candidate(frame, depth_frame, latest_ee_pose):
    ...
    if USE_DEPTH_ASSIST:
        ...
        if grasp_cam_from_depth is not None:
            measurement_mode = f"{point_mode} + depth"
...
if SHOW_TEXT_ON_MAIN_WINDOW:
    depth_status = "Depth assist: ready"
    if depth_frame is None:
        depth_status = "Depth assist: waiting for real depth frame"
    if DEPTH_FRAME_SOURCE == "hp60_sdk" and hp60_camera_failed:
        depth_status = "Depth assist: HP60 SDK failed"
    cv2.putText(
        display,
        depth_status,
